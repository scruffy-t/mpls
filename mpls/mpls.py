...
def __get(name, stype, **kwargs):
    ...
    logger.debug('loaded raw {} file from {}'.format(type, data_url))
...
def use(*args, context=None, style=None, palette=None, **kwargs):
    ...
    # apply specified matplotlib styles
    plt.style.use(args)
    # apply mpls styles
    return plt.style.use(rc(context=context, style=style, palette=palette, **kwargs))
...
def temp(*args, context=None, style=None, palette=None, **kwargs):
    ...
    with plt.style.context(args, after_reset=kwargs.get('reset')):
        # apply mpls styles
        return plt.style.context(rc(context=context, style=style, palette=palette, **kwargs))
